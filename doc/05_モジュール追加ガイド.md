...
sed 's/template/moduleD/g; s/TEMPLATE/MODULED/g' \
    tools/template/target/module_template.h > target/jchg/moduleD/moduleD.h

# ソースファイルをコピーして置換
sed 's/template/moduleD/g' \
    tools/template/target/module_template.c > target/jchg/moduleD/moduleD.c
```
...
# テストファイルをコピーして置換
sed 's/template/moduleD/g; s/Template/ModuleD/g' \
    tools/template/tests/test_module_template.cpp > tests/moduleD/test_moduleD.cpp

# CMakeLists.txt をコピーして置換
sed 's/template/moduleD/g; s/Template/ModuleD/g' \
    tools/template/tests/CMakeLists.txt.template > tests/moduleD/CMakeLists.txt
```
...
```bash
sed 's/template/moduleD/g' \
    tools/template/double/module_stub_template.c > double/moduleD/moduleD_stub.c
```
...
### tools/template/target/module_template.h
...
### tools/template/target/module_template.c
...
### tools/template/tests/test_module_template.cpp
...
### tools/template/tests/CMakeLists.txt.template
...
add_test(NAME Module${MODULE_NAME}Test COMMAND test_${MODULE_NAME})
```
...
### tools/template/double/module_stub_template.c
...
A. `tools/template/` フォルダ内のファイルを編集してください:

```bash
# 例：tools/template/target/module_template.c のテンプレートを編集
vim tools/template/target/module_template.c
```
